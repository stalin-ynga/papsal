...
if($estado!="T"){$where=$where." AND ( tblmarcado.Condicion='$estado' OR tblmarcado.condicion='04')";};
	   ...
	   //
		...
		if($tipo=='SA2'){$it2='SALIDA DRSAU';};
	     ...
	     $sql_guardar="select tblmarcado.Codigo as Codigo,NombreCom,Fecha,Conteo,(CASE Tipo WHEN 'EN1' THEN 'ENTRADA DRSAU' WHEN 'SA1' THEN 'SALIDA REFRIGERIO' WHEN 'EN2' THEN 'ENTRADA REFRIGERIO' WHEN 'SA2' THEN 'SALIDA DRSAU' ELSE NULL END) as Tipo,Papeleta,tblmarcado.Estado,TRUNCATE((((Sueldo/31)/8)/60)*Conteo,2) AS dcto,(CASE Tblusuario.Condicion WHEN '0000000001' THEN 'CAS' WHEN '0000000002' THEN 'NOMBRADO' WHEN '0000000003' THEN 'PROYECTO' WHEN '0000000004' THEN 'TERCEROS' WHEN '0000000005' THEN 'ADSCRITOS' WHEN '0000000006' THEN 'CONTRATADO' ELSE NULL END) as Tipo from tblmarcado,tblusuario WHERE tblusuario.estado='A' and tblusuario.Codigo=tblmarcado.Usuario AND Fecha BETWEEN '$inicio' AND '$final'".$where." ORDER BY  tblusuario.Condicion,tblusuario.NombreCom ;";
...
$objPHPExcel->setActiveSheetIndex(0)
			...
			->setCellValue('N3',  $it2)
                 ...
                 ->setCellValue('G7', 'FECHA Y HORA')
			->setCellValue('S7', 'CONDICION')
			->setCellValue('I7','MIN.ACUMULADO')
			->setCellValue('K7','TIPO MARCADO')
			->setCellValue('M7','DOC/JUSTIFICACION')
			->setCellValue('P7','ESTADO MARCACION')
                 ->setCellValue('U7','DSCTO S/.')
...
foreach($data as $row)
		{
			...
			->setCellValue('S'.$i,$row[8])
            ->setCellValue('U'.$i,$row[7]);     	
...
$objPHPExcel->getActiveSheet()->getStyle('U7:V7')->applyFromArray($styleThinBlackBorderOutline);
