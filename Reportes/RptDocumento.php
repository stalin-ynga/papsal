...
$sql_guardar1="select tblsalida.Codigo,NombreCom,FecSalida,FecRetorno,(select Nombre from tbltipomotivo where TipoMotivo=tbltipomotivo.Codigo) as Tipo,Lugar,(CASE Statuss WHEN '01' THEN 'PENDIENTE' WHEN '02' THEN 'APROBADO' ELSE NULL END) AS Estado
...
$sql_guardar1="select tblvehicular.Codigo,NombreCom,(SELECT Modelo from tblauto where tblauto.Codigo=Vehiculo) as Vehi,(select Nombre from tbltipomotivo where TipoMotivo=tbltipomotivo.Codigo) as Tipo,Fecha,Destino,(CASE Statuss WHEN '01' THEN 'PENDIENTE' WHEN '02' THEN 'APROBADO' ELSE NULL END) AS Estado from tblvehicular,tblusuario WHERE NOT(Statuss='04')
...
$this->Cell(1140,15,'Reportes De Documentos Generados',0,'1LB','C',true);
...
$header = array('Codigo Documento', 'Trabajador', 'Fecha Salida', 'Fecha Retorno', 'Justificacion', 'Lugar', 'Estado');
