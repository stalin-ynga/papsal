...
from tblsalida,tbldetnivel,tblusuario WHERE Usuario=Personal AND Jefe='".$_SESSION['Codigo']."' AND Personal=tblusuario.Codigo AND FecSalida BETWEEN '$inicio' AND '$final' ".$where." ; 
 ";
 
 $sql_guardar2="select COUNT(*) from tblsalida,tbldetnivel,tblusuario WHERE Usuario=Personal AND Jefe='".$_SESSION['Codigo']."' AND Personal=tblusuario.Codigo AND FecSalida BETWEEN '$inicio' AND '$final' ".$where." ; 
 ";
...
from tbltardanza,tbldetnivel,tblusuario WHERE Usuario=Personal AND Jefe='".$_SESSION['Codigo']."' AND Personal=tblusuario.Codigo AND Fecha BETWEEN '$inicio' AND '$final' ".$where." ; 
 ";
 
 $sql_guardar2="select COUNT(*) from tbltardanza,tbldetnivel,tblusuario WHERE Usuario=Personal AND Jefe='".$_SESSION['Codigo']."' AND Personal=tblusuario.Codigo AND Fecha BETWEEN '$inicio' AND '$final' ".$where." ; 
 ";
       $res1 =mysql_query($sql_guardar1,$enlace); 
	   $res2 =mysql_query($sql_guardar2,$enlace); 
	   while ($reg2 = mysql_fetch_array($res2))
...
from tblvehicular,tbldetnivel,tblusuario WHERE Usuario=Personal AND Jefe='".$_SESSION['Codigo']."' AND Personal=tblusuario.Codigo AND Fecha BETWEEN '$inicio' AND '$final' ".$where." ; 
 ";
 
 $sql_guardar2="select COUNT(*) from tblvehicular,tbldetnivel,tblusuario WHERE Usuario=Personal AND Jefe='".$_SESSION['Codigo']."' AND Personal=tblusuario.Codigo AND Fecha BETWEEN '$inicio' AND '$final' ".$where." ; 
 ";
       $res1 =mysql_query($sql_guardar1,$enlace); 
	   $res2 =mysql_query($sql_guardar2,$enlace); 
	   while ($reg2 = mysql_fetch_array($res2))
 ...
 //LLAMA A TODAS LAS PAPELETAS DE SALIDA DEL DIA
	$sql_guardar1="select tblsalida.Codigo,NombreCom,FecSalida,FecRetorno,(select Nombre from tbltipomotivo where TipoMotivo=tbltipomotivo.Codigo) as Tipo,Lugar,Fundamento,Observacion,(CASE Statuss WHEN '01' THEN 'PENDIENTE' WHEN '02' THEN 'APROBADO' ELSE NULL END) AS Estado
from tblsalida,tbldetnivel,tblusuario WHERE Usuario=Personal AND Jefe='".$_SESSION['Codigo']."' AND Personal=tblusuario.Codigo AND FecSalida BETWEEN CONCAT(CONVERT(CURDATE(),CHAR),' 00:00:00') AND CONCAT(CONVERT(CURDATE(),CHAR),' 23:59:00'); 
 ";
 $sql_guardar2="SELECT COUNT(*) from tblsalida,tbldetnivel,tblusuario WHERE Usuario=Personal AND Jefe='".$_SESSION['Codigo']."' AND Personal=tblusuario.Codigo AND FecSalida BETWEEN CONCAT(CONVERT(CURDATE(),CHAR),' 00:00:00') AND CONCAT(CONVERT(CURDATE(),CHAR),' 23:59:00'); 
 ";
...
function Aprueba(){
	...
	document.f2.action='Aprueba.php';
	document.f2.submit();
	}
...
<body>
<center>
<form name="f2" id="f2" method="post" action="PorAprobar.php"  onSubmit="return validar(this);" class="ww Estilo9">
  <script type="text/javascript">
...
<table width="976" border="0">
  <tr>
    <td width="976" bgcolor="#33CC00" class="Estilo9">Papeletas Pendientes Para Su Aprobacion</td>
  </tr>
</table>
<table width="976" border="0">
  <tr>
    <td width="976" bgcolor="#0033FF" class="titulo_cabecera">&nbsp;</td>
    </tr>
</table>
<table width="985" border="0">
  <tr>
    <td width="31" class="titulo_cabecera Estilo9"><span>Desde:</span></td>
    <td width="185"><input name="inicio" type="text" class="tb6 a" id="inicio"   readonly="readonly"/></td>
    <td width="32" class="titulo_cabecera Estilo9">Hasta: </span></td>
    <td width="185">
    ...
    <td width="206"><label for="select"></label>
  ...
  <tr>
    ...
    <td>&nbsp;</td>
    <td>&nbsp;</td>
    <td><input type="submit" name="button" id="button" value="FILTRAR" onclick="asigna(this); "  /></td>
    <td>&nbsp;</td>
    <td><p>&nbsp;</p>
      <p>&nbsp;</p></td>
    <td><input type="button" name="aprueba" id="aprueba" value="  APROBAR PERMISOS    " onclick="Aprueba();" /></td>
  </tr>
</table>
</div>
		...
		<thead>
			<tr>
				...
				{		
	             ...
	             if($tipo=='01'){		
			echo"	<th width='120'><h3>Codigo</h3></th>";
			echo"	<th width='150'><h3>Usuario</h3></th>";
			echo"	<th width='159'><h3>Fecha Salida</h3></th>";
 ...
 <input name="ip" type="hidden" id="ip" value="<? echo $ipcliente?>"  />
  <input name="usuario" type="hidden" id="usuario" value="<? echo $_SESSION["Codigo"] ?>"  />
